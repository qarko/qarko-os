...
export const defaultRules: ApprovalRule[] = [
  ...
  },
  {
    id: "file-creation",
    label: "파일 생성",
    description: "로컬 프로젝트 파일과 산출물 생성",
    requiresApproval: false,
    risk: "medium",
  },
  {
  ...
  },
  {
    id: "external-posting",
    label: "외부 게시",
    description: "SNS, 블로그, 커뮤니티에 공개 게시",
    requiresApproval: true,
    risk: "high",
  },
  {
    id: "paid-api",
    label: "유료 API 사용",
    description: "비용이 발생할 수 있는 API 호출",
    requiresApproval: true,
    risk: "high",
  },
  {
    id: "plugin-install",
    label: "플러그인 설치",
    description: "외부 기능 또는 권한이 필요한 확장 설치",
    requiresApproval: true,
    risk: "medium",
  },
  {
  ...
  },
  {
    id: "community-share",
    label: "커뮤니티 공유",
    description: "워크플로, 플러그인, 템플릿 공개 공유",
    requiresApproval: true,
    risk: "medium",
  },
];
...
export const automationPolicies: AutomationPolicy[] = [
  {
    mode: "manual",
    label: "수동 모드",
    description: "AI는 제안과 초안만 만들고 의미 있는 실행은 모두 승인 후 진행합니다.",
  ...
  },
  {
    mode: "assisted",
    label: "보조 모드",
    description: "안전한 내부 작업은 자동으로 진행하고, 외부 실행과 고위험 작업은 승인받습니다.",
    allowedSummary: "리서치, 초안, 체크리스트, 작업 정리",
  ...
  },
  {
    mode: "automation",
    label: "자동화 모드",
    description: "낮은 위험의 내부 워크플로는 자동 실행하고 큰 결정만 승인받습니다.",
    allowedSummary: "내부 워크플로 자동 실행",
    approvalSummary: "외부 게시, 비용, 삭제, 프로덕션 변경 승인 필요",
  ...
  },
  {
    mode: "custom",
    label: "커스텀 모드",
    description: "작업 유형별로 승인 필요 여부를 직접 조절합니다.",
...
const baseRoles: AgentRole[] = [
  {
    ...
    status: "running",
    currentFocus: "이번 주 가장 중요한 실행 순서를 재정렬 중",
  },
  {
    id: "researcher",
    name: "Researcher",
    ...
    status: "running",
    currentFocus: "검증에 필요한 참고자료와 리스크 수집",
  },
  {
    id: "marketer",
    name: "Growth Marketer",
    mission: "유통, 콘텐츠, 수익화 가설 설계",
    status: "planned",
  ...
  },
  {
    id: "reviewer",
    name: "QA / Reviewer",
    mission: "결과물 품질과 승인 위반 여부 검토",
    status: "needs_approval",
    currentFocus: "공개 실행 전 승인 조건 점검",
  },
];
...
export const projects: Project[] = [
    ...
    tasks: [
      {
        ...
        description: "사용자의 아이디어를 고객, 문제, 제안 가치 기준으로 압축",
        status: "completed",
        roleId: "chief",
      ...
      },
      {
        id: "task-2",
        title: "경쟁 대안 리서치",
        description: "이미 존재하는 도구, 서비스, 커뮤니티 반응 조사",
        status: "running",
        roleId: "researcher",
        approvalRequired: false,
      },
      {
        id: "task-3",
        title: "첫 공개 테스트 승인",
        description: "외부에 공개될 메시지를 검토하고 승인",
        status: "needs_approval",
        roleId: "reviewer",
  ...
  {
    id: "project-local-service",
    name: "Local Service Growth System",
    idea: "로컬 사업자가 반복 사용할 수 있는 성장 운영 시스템 만들기",
    status: "planned",
    automationMode: "manual",
    goal: {
      id: "goal-local",
      title: "서비스 패키지와 영업 메시지 초안 만들기",
      metric: "잠재 고객 20명에게 보여줄 제안서 준비",
      status: "planned",
    },
    workflow: {
      id: "workflow-local",
      title: "Offer to Outreach",
      description: "제안 설계부터 첫 영업 자료까지 정리",
      stages: [
        { id: "stage-local-1", title: "고객군 정의", status: "planned", ownerRoleId: "chief" },
        { id: "stage-local-2", title: "제안서 초안", status: "planned", ownerRoleId: "marketer" },
      ],
    },
    tasks: [
      {
    ...
    roles: baseRoles,
    risks: ["고객군이 넓으면 메시지가 흐려질 수 있음"],
    nextAction: "고객군을 하나로 좁히고 제안서 초안을 생성하세요.",
  },
  {
    id: "project-product-studio",
    name: "Digital Product Studio",
    idea: "작은 디지털 제품 아이디어를 빠르게 검증하고 출시하기",
    status: "blocked",
    automationMode: "custom",
    goal: {
      id: "goal-product",
      title: "첫 디지털 제품 MVP 범위 결정",
      metric: "3개 기능 이하의 판매 가능한 MVP 정의",
      status: "blocked",
    },
    workflow: {
      id: "workflow-product",
      title: "MVP Scope Review",
      description: "기능 욕심을 줄이고 검증 가능한 범위로 정리",
      stages: [
        { id: "stage-product-1", title: "기능 후보 정리", status: "completed", ownerRoleId: "chief" },
        { id: "stage-product-2", title: "범위 축소", status: "blocked", ownerRoleId: "reviewer" },
      ],
    },
    tasks: [
      {
        id: "task-product-1",
        title: "기능 범위 승인",
        description: "MVP 기능을 3개 이하로 줄일지 결정",
        status: "blocked",
        roleId: "reviewer",
        approvalRequired: true,
      },
    ...
    roles: baseRoles,
    risks: ["초기 제품 범위가 커질 위험", "유료 기능 검증 전 개발 비용 증가"],
    nextAction: "MVP 기능 목록을 승인하거나 수정하세요.",
  },
];
...
export const approvals: Approval[] = [
  {
    ...
    whatWillHappen: "Growth Marketer가 작성한 짧은 검증 메시지를 외부 채널에 게시할 준비를 합니다.",
  ...
  {
    id: "approval-plugin",
    projectId: "project-product-studio",
    action: "Local File Operator 활성화",
    whatWillHappen: "프로젝트 산출물을 로컬 파일로 저장할 수 있게 됩니다.",
...
export const artifacts: Artifact[] = [
  ...
  },
  {
    ...
    title: "MVP 범위 리뷰",
    type: "review",
    summary: "기능 범위가 과한 지점과 줄일 수 있는 항목을 표시",
    createdAt: "2026-05-16 22:15",
  },
];
...
export const plugins: Plugin[] = [
  ...
  },
  {
    ...
    capability: "웹 리서치와 참고자료 수집을 자동화",
  ...
  {
    id: "plugin-drive",
    name: "Google Drive Artifacts",
    category: "community",
    capability: "산출물을 Drive 폴더에 정리",
    permissions: ["파일 생성", "Drive 연결"],
    workflows: ["산출물 저장", "문서 공유"],
    risk: "medium",
    enabled: false,
  },
];
